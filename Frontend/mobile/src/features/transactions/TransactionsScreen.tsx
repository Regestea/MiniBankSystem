...
/** Feature: transactions — full history with type filter + search (live API). */
  ...
  return (
    <div className="mobile-screen">
      ...
      <Card>
        ...
        <Input
          label="Search"
          placeholder="Name, account, title…"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
        />
      </Card>
      <Card>
        {error ? <p role="alert">{error}</p> : <TransactionList transactions={visible} />}
      </Card>
    </div>
  );
}
